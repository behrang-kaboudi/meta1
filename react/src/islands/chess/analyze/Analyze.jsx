...
export default function Analyze({ pgnText }) {
  ...
  const { ref: boardPart } = useResizeObserver({
    onResize: ({ width, height }) => {
      console.log('useResizeObserver', width);
  ...
  function afterBoardMove(moveObj) {
    ...
    setBoardAndPgnViewer(changed.enrichedPgn.map[changed.newMoveObj.enriched.id]);
    setPgnViewerVersion((x) => x + 1);
    //todo set enriched move and PGN
  }
  function setBoardAndPgnViewer(enrichedMove, position = null) {
    setEnrichedMove(enrichedMove);
    setMoveObj({ position, enrichedMove });
  }
      ...
      <div className={clsx(styles.mainDiv)}>
        <div>
          <div ref={boardPart} className={clsx(styles['board-part'])}>
            ...
            <div>
              <EvalGauge reverse={gaugeReverse} value={2} />
            </div>
          </div>
        </div>
        <Box
          sx={{
            mt: [1, 1, 0],
            ml: [0, 0, 1],
            height: [200, boardPartHeight],
            border: 1,
            borderRadius: 1,
          }}
        >
          <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%' }} className={clsx()}>
            <Box
              sx={{
                maxHeight: '100%',
                width: '100%',
            ...
            <Box sx={{ width: `calc(100% - ${scrollbarWidth}px)`, pt: 1, pb: 1 }}>
              <Box sx={{}} className={clsx('d-flex justify-content-around ')}>
