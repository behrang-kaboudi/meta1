...
import { enrichPgn, addMoveAfterParent } from "@meta/enrich-pgn";
...
export default function Analyze({ pgnText }) {
  ...
  function afterBoardMove(moveObj, type = "manual") {
    ...
    setBoardAndPgnViewer(
      changed.enrichedPgn.map[changed.newMoveObj.enriched.id],
      "",
      type
    );
  ...
  function onClickEnd() {
    let nextMove =
      enrichedPgn.game?.moves[enrichedPgn.game?.moves.length - 1] || null;
    if (nextMove) setBoardAndPgnViewer(nextMove);
  }
  ...
  function onAnalyze(line) {
    ...
    setEvalValue(value);
    try {
      if (line.enrichedMoves?.length > 2) {
        setArrows([
          {
        ...
        >
          ...
          >
            ...
            >
              <Box
                sx={{}}
                className={clsx("d-flex justify-content-around  pb-1")}
              >
