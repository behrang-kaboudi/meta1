...
const { io } = require("../../routes/mainRout");
const fs = require("fs");
...
const { router: gameRoot } = require("../../routes/game");
...
class Pairing {
  ...
  async pair() {
    ...
    if (this.rd != 0) {
      ...
      let unresult = this.players.find(
        (p) => p.results.length != p.gameIds.length
      );
      if (unresult) {
        await this.dispose();
        return false;
  ...
  async updateRoundResults() {
    ...
    for (let i = 0; i < this.players.length; i++) {
      ...
      if (game.whiteResult || game.blackResult) {
        pl.results[pl.gameIds.length - 1] =
          pl.userName == game.whiteUserName
            ? game.whiteResult
            : game.blackResult;
  ...
  doDutch() {
    ...
    let process = spawn("./java/jdk/bin/java.exe", [
      "-jar",
      "./swp/javafo.jar",
      this.inputFile,
      "-p",
...
pairing.getPairingObj = async function (tourId) {
  let tour = await Swiss.findOneAndUpdate(
    { _id: tourId, inUse: false },
    { $set: { inUse: true } },
...
pairing.putUsersIndb = async function (tour) {
  ...
  for (let i = 0; i < tour.players.length; i++) {
    ...
    let prePlayer = await swissPlayer.findOne({
      userName: newpl.userName,
      tournamentId: newpl.tournamentId,
    });
...
trfx = {
  ...
  getOpponentNum: function (player, allPlayers, rd) {
    let number = allPlayers.findIndex(
      (pl) => pl.opponents[rd] == player.userName
    );
    if (number < 0) {
      number = "0000";
    } else {
