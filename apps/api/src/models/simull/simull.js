...
Simull.api.myGames = async function (userName) {
  let simulls = await Simull.find({ status: { $ne: "finished" } }).or([
...
Simull.api.available = async function (userName) {
  let simulls = await Simull.find({ status: "open" }).and({
...
Simull.api.start = async function (userName, simId) {
  let simull = await Simull.findOne({
    status: "open",
    _id: simId,
    creatorUserName: userName,
  });
...
gameRoot.event.on("gameFinished", async (game) => {
  if (game.tournamentType == "simull") {
    ...
    if (allFinished) {
      let simull = await Simull.updateOne(
        { status: "closed", _id: game.tournamentId },
        { status: "finished" }
      );
