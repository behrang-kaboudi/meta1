...
async function ensureRepo() {
  ...
  if (!fs.existsSync(gitDir)) {
    if (!REMOTE)
      throw new Error("DEPLOY_REMOTE_URL is required for first-time setup");
    console.log("📦 First-time setup: cloning repository…");
    await run(
      "git",
      ["clone", "--branch", BRANCH, "--single-branch", REMOTE, REPO],
      {
        cwd: parent,
      }
    );
  ...
  try {
    ...
    if (!REMOTE)
      throw new Error("origin is missing and DEPLOY_REMOTE_URL not provided");
