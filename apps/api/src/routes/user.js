...
const userValidator = require("../models/user/validate");
const ut = require("../models//utility");
const user = require("../models/user/user");
const preregister = require("../models/user/preregister");
...
router.post("/login/", loginLimiter, async (req, res) => {
  ...
  let isHash;
  if (dbUser)
    isHash = await verifyPassword(dbUser.passwordHash, req.body.password);
  if (!dbUser || !isHash) {
    myRes.state = false;
...
router.post("/recovery/", (req, res) => {
  ...
  user.findOne({ email: req.body.email }).then((dbUser) => {
    ...
    let time = Date.now();
    dbUser.recoveryLink =
      "tt-tt" + ut.getRndInteger(123456, 987654) + "tt-tt" + time;
...
router.post("/register/", registerLimiter, (req, res) => {
  ...
  preregister.isInDb(req.body.email, req.body.userName).then(async (dbreg) => {
    if (dbreg.ans) {
      ...
      tempUser.link =
        tempUser.userName + tempUser.time + ut.getRndInteger(123456, 987654);
