...
router.ioF = async function (data, ack, userData) {
  if (!data) return;
  await router.io[data.signal](data, ack, userData);
};
router.io.opens = async function () {
  ...
  hub.emit("openChalengs", chalenges);
};
router.io.byFriends = async function (data, ack, userData) {
...
router.io.cancel = async function (data, ack, userData) {
  let answer = await chalenge.updateOne(
    { _id: data.id },
    { $set: { status: "rejected" } }
  );
  answer = await chalenge.findById(data.id);
  // hub.to(answer.opponentUserName).emit('chalengeReq', 'chalengeReq');
  ack(answer);
};
router.io.accept = async function (data, ack, userData) {
...
async function acceptChalenge(accepterUserData, chalengeId, ack) {
  ...
  hub.to("user-" + dbChalenge.requsterUserName).emit("goToPlayPage", page);
  hub.emit("newGameCreated", gm);
  hub.emit("openChalengsUpdate");
  ack(gm);
}
...
router.io.create = async function (data, ack, userData) {
  ...
  if (!("opponentUserName" in data.chalenge)) {
    openChaleng = true;
    let preSameChaleng = await getSameOpenChalenge(
      data.chalenge.requsterUserName,
      data.chalenge
    );
    if (preSameChaleng) {
  ...
  hub
    .to("user-" + data.chalenge.opponentUserName)
    .emit("chalengeReq", "chalengeReq");
  ack(answer);
  if (openChaleng) {
    hub.emit("openChalengsUpdate");
  }
