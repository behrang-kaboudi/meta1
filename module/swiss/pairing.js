...
const { router: gameRoot } = require('../../rout/game');
...
Swiss.updateMany({ $or: [{ status: 'open' }, { status: 'closed' }] }, { inUse: false }).then(
  function () {
    // console.log('ok',);
  },
);
    ...
    for (let i = 0; i < this.players.length; i++) {
      ...
      if (game.whiteResult || game.blackResult) {
        pl.results[pl.gameIds.length - 1] =
          pl.userName == game.whiteUserName ? game.whiteResult : game.blackResult;
