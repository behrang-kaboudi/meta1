    ...
    Pgn: class {
        ...
        putMove(moveObj, moveIndex, varIndex) {
            ...
            if (moveObj.promotion) {
                currentM.moveObj = chess.move({ from: moveObj.sq1.name, to: moveObj.sq2.name, promotion: moveObj.promotion[1] })
            } else {
            ...
            if (currentM.parentMove.ans.length == 0 || !currentM.parentMove.ans[0]) {
                ...
                if (currentM.parentMove.ans.length > 2) {
                    ...
                    this.varMatris.push(oneVar);
                    let creatSubVar = (variant) => {
                        let curentVar = variant;
                        let nodeMove = null;
                        for (let i = curentVar.moves.length - 1; i > 0; i--) {
                            const move = curentVar.moves[i];
                            let inParentIndex = move.parentMove.ans.findIndex(m => m.moveObj.san == move.moveObj.san);
                            if (inParentIndex == move.parentMove.ans.length - 1) continue;
                            nodeMove = move.parentMove.ans[inParentIndex + 1];
                            break;
                        }
                        return creatVariantFromNodeMove(nodeMove)
                    }

                    let newVarMatris = [];
                    newVarMatris.push(creatVariantFromNodeMove(this.preFirstMove))
                    for (let i = 0; i < this.varMatris.length - 1; i++) {
                        const var1 = this.varMatris[i];
                        newVarMatris.push(creatSubVar(var1))
                    }

                    function creatVariantFromNodeMove(nodeMove) {
                        let variant = new pgn.Variant();
                        let currentMove = nodeMove;
                        while (currentMove) {
                            variant.moves.push(currentMove);
                            currentMove = currentMove.parentMove;
                            if (!currentMove) break;
                        }
                        variant.moves.reverse();
                        currentMove = nodeMove;

                        if (currentMove && currentMove.ans.length > 0 && currentMove.ans[0]) {
                            while (currentMove.ans[0]) {
                                variant.moves.push(currentMove.ans[0]);
                                currentMove = currentMove.ans[0];
                            }
                        }
                        return variant;
                    }
