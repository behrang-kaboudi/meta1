...
function BkBoard(parentId, obj) {
    ...
    // bkChessBoardConsts.names = {
    //     sqAttrForName: 'sqname' + parentId,
    //     sqAttrForMask: 'maskname' + parentId,
    //     sqAttrForPiceName: 'picename' + parentId,
    //     imgAttrForParent: 'sqName' + parentId,
    // }
    ...
    bd.setPositionMulti = function (fen) {
        bd.clearPositionMulti();
        bd.settings.fen = fen;
    }
    bd.clearPositionMulti = function () {
        for (var i = 0; i < 64; i++) {
            let sq = document.getElementById(bkChessBoardConsts.SQUARE_COORDINATES[i] + parentId);
            // console.log(sq);
    ...
    bd.addAllEvent = function (svgObg) {
        ...
        svgObg.addEventListener('mousedown', function (e) {
            ...
            if (bd.settings.canMove) {
                ...
                function isCorrectMove(toSq) {
                    bd.moveObj = {
                        from: bd.firstSqMove,
                        to: toSq,
                    };
                    let isCoorect = consts.chessMove(bd, bd.moveObj);
                    if (!isCoorect) {
                ...
                if (!bd.firstSqMove) {
                    ...
                    bd.piceClone.onmouseup = function (e) {
                        ...
                        // new
                        if (bd.settings.engine == 'chess') {

                            let corectM = isCorrectMove(dropdSq);
                            if (!corectM) {
                                document.body.removeChild(bd.piceClone);
                                bd.piceClone = null;
                                return;
                    ...
                    if (bd.settings.engine == 'chess') {
                        let corectM = isCorrectMove(sq);
                        if (!corectM) return;
                    }

                    bd.secSqMove = sq;
                    bd.setMask(bd.secSqMove, 'move-mask');
                    //  console.log(bd.firstSqMove, bd.secSqMove);
                    // bd.doMove(bd.firstSqMove, bd.secSqMove); to do
                    if (bd.Funcs.moveEnd) bd.Funcs.moveEnd();
                }
    ...
    bd.doPromotion = function (pice) {
        bd.moveObj.promotion = pice;
        consts.chessMove(bd, bd.moveObj)
        let p = document.getElementById(`${parentId}promotionw`);
        p.classList.remove('d-none');
        p.classList.add('d-none');
        p = document.getElementById(`${parentId}promotionb`);
        p.classList.remove('d-none');
        p.classList.add('d-none');
        if (bd.Funcs.moveEnd) bd.Funcs.moveEnd();
    ...
    bd.showPromotion = function (side) {
        bd.settings.canMove = false;
        let p = document.getElementById(`${parentId}promotion${side}`);
        p.classList.remove('d-none');
    }
    // moves
...
`;
    ...
    bd.creatPromotion = function () {
        let row = document.createElement('div');
        row.innerHTML = `
        <div style="z-index: 150; margin-top: 10px" class="   w-100 d-flex flex-column my-1 " style="width: 100%;height:1000px">
            <div id="${parentId}promotionb" class="picPromotion py-12 d-flex d-none justify-content-center">
                <img onclick="bd.doPromotion('n')" src="/public/components/bk-chessboard/pices/bn.svg" class="bg-light mx-1 rounded-2" style="width: 14%; border: 1px solid; " />
                <img onclick="bd.doPromotion('b')" src="/public/components/bk-chessboard/pices/bb.svg" class="bg-light mx-1 rounded-2" style="width: 14%; border: 1px solid; " />
                <img onclick="bd.doPromotion('r')" src="/public/components/bk-chessboard/pices/br.svg" class="bg-light mx-1 rounded-2" style="width: 14%; border: 1px solid; " />
                <img onclick="bd.doPromotion('q')" src="/public/components/bk-chessboard/pices/bq.svg" class="bg-light mx-1 rounded-2" style="width: 14%; border: 1px solid; " />
            </div>
            <div id="${parentId}promotionw" class="picPromotion justify-content-center py-12 d-flex d-none" style="width: 100%">
                <img onclick="bd.doPromotion('n')" src="/public/components/bk-chessboard/pices/wn.svg" class="bg-light mx-1 rounded-2" style="width: 14%; border: 1px solid; " />
                <img onclick="bd.doPromotion('b')" src="/public/components/bk-chessboard/pices/wb.svg" class="bg-light mx-1 rounded-2" style="width: 14%; border: 1px solid; " />
                <img onclick="bd.doPromotion('r')" src="/public/components/bk-chessboard/pices/wr.svg" class="bg-light mx-1 rounded-2" style="width: 14%; border: 1px solid; " />
                <img onclick="bd.doPromotion('q')" src="/public/components/bk-chessboard/pices/wq.svg" class="bg-light mx-1 rounded-2" style="width: 14%; border: 1px solid; " />
            </div>
        </div>
        `;
        row.classList.add('bk-row');
        bd.parent.prepend(row);
    }
