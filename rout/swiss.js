...
async function getPrimaryGamedData(players) {
    ...
    for (let i = 0; i < gameIds.length; i++) {
        const id = gameIds[i];
        let g = await Game.findOne({ _id: id }, 'blackUserName whiteUserName blackResult whiteResult tournamentRound boardNo acceptedTime');
        // games.push({ id: g })
        games.push(g)
    }
    return games;
...
pairing.event.on('finishPairing', async function (data) {
    ...
    io.in('swiss-' + data.id)
        .emit('data', { sender: 'finishPairing', root: 'swiss-' + data._id, data: await sendDataCreator(data) });
})
pairing.event.on('gameFinished', async function (tour) {
    ...
    io.in('swiss-' + tour._id)
        .emit('data', { sender: 'gameFinished', root: 'swiss-' + tour._id, data: await sendDataCreator(tour) });
})
...
rout.event.on(eventsNames.changed, async (tour) => {
    io.in('swiss-' + tour._id).emit('data', { root: 'swiss-' + tour._id, data: await sendDataCreator(tour) });
    io.in('swiss').emit('update', { root: 'swiss' });
})
...
rout.io.tour = async (data, ack, userData) => {
    ...
    io.in(userData.socket.id).emit('data', { sender: 'rout.io.tour', root: 'swiss-' + data.id, data: await sendDataCreator(tour) });
}
...
rout.io.upComing = async function (data, ack, userData) {
    let tours = await swissTournament.find({ status: 'open', players: { "$nin": [userData.userName] } });
    io.in(userData.socket.id).emit('upComing', { root: 'swiss', data: tours });
}
rout.io.onGoing = async function (data, ack, userData) {
    let tours = await swissTournament.find({ status: 'closed', players: { "$nin": [userData.userName] } });
...
rout.io.myGames = async function (data, ack, userData) {
    ...
    if (!player) {
        ...
        for (let i = 0; i < player.gameIds.length; i++) {
            ...
            g = await rtGame.io.getGameData(gId)
            gms.push(g)
        }
        ack(gms);
    }
...
rout.io.liveGame = async function (data, ack, userData) {
    ...
    let game = await rtGame.api.getTourLiveGame({ tournamentId: data.id, tournamentRound: newSwiss.round, boardNo: data.gameIndex })
    if (game) {
        ack(game);
    }
    ack(false);
