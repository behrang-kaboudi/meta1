...
setInterval( async function () {
    ...
    games.forEach(async g => {
        rout.io.getGameData(g.id)
    });
},10000 )
rout.io = {};

rout.io.userSwissGames = async function (tour) {
    let games = await game.find({ tournamentId: tour.id }).or([{ blackUserName: tour.userName }, { whiteUserName: tour.userName }]);
    return games;
};
...
rout.io.searchInGames = async (data) => {
    ...
    if (data.setColor) {
        ...
        let allUserBlack = craetPlayersQueryArray(data.blackPlayers.concat(data.whitePlayers), 'blackUserName');
        let allUserWhite = craetPlayersQueryArray(data.blackPlayers.concat(data.whitePlayers), 'whiteUserName');
        ...
        if (data.blackPlayers[0] && data.whitePlayers) {
            ...
            games = games.filter(g => mergedNames.includes(g.blackUserName) || mergedNames.includes(g.whiteUserName))
        }
    }
    // if (data.result != 'all') {
...
rout.api.getSimullGame = async function (simull, user) {
    ...
    let gm = await game.findOne({ tournamentId: simull._id, whiteUserName: wUser, blackUserName: bUser });
    return gm;
};
...
rout.api.getTournamentGames = async function (tourId) {
...
rout.api.userSwissGames = async function (tourId,userName) {
    let games = await game.find({ tournamentId: tourId }).or([{ blackUserName: userName }, { whiteUserName: userName}]);
    return games;
};
module.exports = rout;
...
function setForEndGame(game) {
    removeTimer(game);
    rout.event.emit('gameFinished', addParamsToGame(game));
}
